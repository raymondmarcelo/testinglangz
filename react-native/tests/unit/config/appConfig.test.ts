...
describe('appConfig', () => {
  afterEach(() => {
    delete process.env.RN_PUBLIC_APP_NAME;
    delete process.env.RN_PUBLIC_APP_ENV;
    delete process.env.RN_PUBLIC_API_BASE_URL;
  });
  ...
  it('uses fallback values when extras and env vars are missing', () => {
    ...
    expect(config).toEqual({
      appName: 'Template Repo Mobile React Native',
  ...
  it('uses environment variables for environment and api base url', () => {
    process.env.RN_PUBLIC_APP_NAME = 'Template Staging';
    process.env.RN_PUBLIC_APP_ENV = 'staging';
    process.env.RN_PUBLIC_API_BASE_URL = 'https://staging.api.example.com';
